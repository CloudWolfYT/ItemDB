...
execute store result score in itemdb run data get storage itemdb temp.tag.id
execute if score in itemdb matches 1.. run function itemdb:versioner/item/update
execute unless score in itemdb matches 0 unless data storage itemdb temp.tag.has_id as @e[type=armor_stand,tag=itemdb] run function itemdb:versioner/item/vanilla
